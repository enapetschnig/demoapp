...
interface EItem {
  ...
  unit_price: number; purchase_price?: number; markup_percent?: number; discount_percent?: number; vat_rate: number; time_minutes?: number;
}
...
export default function DokumentEditor() {
  ...
  const { data: template } = useDocTemplate(isNew ? templateId ?? undefined : undefined);
  ...
  useEffect(() => {
    ...
    // Neues Dokument aus Vorlage vorbelegen
    if (isNew && !initialized && templateId && template) {
      setBaseType(template.base_type);
      setIntroText(template.intro_text ?? "");
      setOutroText(template.outro_text ?? "");
      const its = Array.isArray(template.items) ? (template.items as Record<string, unknown>[]) : [];
      setItems(its.map((it) => ({
        key: nk(), kind: (it.kind as EItem["kind"]) ?? "artikel",
        article_id: (it.article_id as string) ?? null, service_id: (it.service_id as string) ?? null,
        name: (it.name as string) ?? "", description: (it.description as string) ?? "",
        quantity: Number(it.quantity ?? 1), unit: (it.unit as string) ?? "Stk",
        unit_price: Number(it.unit_price ?? 0), purchase_price: Number(it.purchase_price ?? 0),
    ...
    // Neues Dokument ohne Vorlage: Einleitungs-/Schlusstext vorbelegen
    if (isNew && !initialized && !templateId && texts.length) {
  ...
  }, [isNew, loaded, initialized, texts, baseType, template, templateId]);
  ...
  const calcItems: CalcItem[] = items.map((i) => ({
    ...
    markup_percent: i.markup_percent, discount_percent: i.discount_percent, vat_rate: i.vat_rate, time_minutes: i.time_minutes,
  }));
  ...
  const buildPayload = () => ({
    ...
    items: items.map<SaveItem>((i) => ({
      ...
      unit_price: i.unit_price, purchase_price: i.purchase_price,
      markup_percent: i.markup_percent, discount_percent: i.discount_percent,
  ...
  const saveAsTemplate = async () => {
    const name = window.prompt("Name der Vorlage:", subject || docLabel(baseType));
    if (!name) return;
    try {
      await createTemplate.mutateAsync({
        name, base_type: baseType, intro_text: introText, outro_text: outroText,
        items: items.map((i) => ({
          kind: i.kind, article_id: i.article_id ?? null, service_id: i.service_id ?? null,
          name: i.name, description: i.description, quantity: i.quantity, unit: i.unit,
          unit_price: i.unit_price, purchase_price: i.purchase_price, markup_percent: i.markup_percent,
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_320px]">
        ...
        <Card className="space-y-4 p-6">
          ...
          <div>
            ...
            <div className="overflow-x-auto rounded-md border">
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-left text-xs">
                  <tr>
                    <th className="w-10 p-2">Pos</th><th className="w-16 p-2">Menge</th><th className="w-16 p-2">Einheit</th>
                    <th className="p-2">Bezeichnung</th><th className="w-24 p-2 text-right">Einzelpreis</th>
                    <th className="w-16 p-2 text-right">Aufschlag&nbsp;%</th><th className="w-16 p-2 text-right">Rabatt&nbsp;%</th>
                    <th className="w-14 p-2 text-right">MwSt</th><th className="w-24 p-2 text-right">Gesamt</th><th className="w-8 p-2"></th>
                  </tr>
                </thead>
                <tbody>
                  {items.length === 0 && <tr><td colSpan={10} className="p-4 text-center text-muted-foreground">Noch keine Positionen — rechts aus dem Artikelstamm hinzufügen.</td></tr>}
                  {items.map((it, idx) => it.kind === "titel" || it.kind === "text" ? (
                    <tr key={it.key} className="border-t bg-muted/20">
                      <td className="p-2"></td>
                      <td colSpan={7} className="p-2">
                        <Input value={it.name} placeholder={it.kind === "titel" ? "Titel / Zwischenüberschrift" : "Freitext"} onChange={(e) => updateItem(it.key, { name: e.target.value })} className="h-8 font-medium" />
                      </td>
                      <td></td>
                    ...
                    <tr key={it.key} className="border-t align-top">
                      ...
                      <td className="p-2"><Input type="number" value={it.markup_percent ?? 0} onChange={(e) => updateItem(it.key, { markup_percent: Number(e.target.value) || 0 })} className="h-8 text-right" /></td>
                      <td className="p-2"><Input type="number" value={it.discount_percent ?? 0} onChange={(e) => updateItem(it.key, { discount_percent: Number(e.target.value) || 0 })} className="h-8 text-right" /></td>
                      ...
                      <td className="p-2 text-right font-medium">{fmtEUR(lineNet(it))}</td>
                      <td className="p-2"><button onClick={() => removeItem(it.key)}><Trash2 className="h-4 w-4 text-muted-foreground hover:text-destructive" /></button></td>
                    </tr>
                  ))}
                </tbody>
            ...
            {/* Summen */}
            <div className="mt-3 ml-auto w-full max-w-sm space-y-1 text-sm">
              {(discountPercent > 0 || !isFinalized) && (
                <div className="flex justify-between"><span className="text-muted-foreground">Zwischensumme</span><span>{fmtEUR(calc.positionsNet)}</span></div>
              )}
              <div className="flex items-center justify-between gap-2">
                <span className="flex items-center gap-2 text-muted-foreground">
                  Rabatt
                  <Input type="number" value={discountPercent} disabled={isFinalized}
                    onChange={(e) => setDiscountPercent(Number(e.target.value) || 0)}
                    className="h-7 w-16 text-right" /> %
                </span>
                <span className="text-muted-foreground">{calc.docDiscount > 0 ? `– ${fmtEUR(calc.docDiscount)}` : ""}</span>
              </div>
